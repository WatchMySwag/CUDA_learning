...
int main() {
    future<int> wynik = async(launch::async, pobierzDaneZInternetu, "http://example.com/dane");//async tworzy wątek, który wykonuje funkcję pobierzDaneZInternetu z podanym argumentem. Future będzie czekał na wynik tej funkcji.
    future<int> wynik2 = async(launch::async, pobierzDaneZInternetu, "http://example.com/inne_dane"); // możemy uruchomić kilka asynchronicznych zadań jednocześnie, a future będzie czekał na każdy z nich osobno.
