...
void przetwarzaj_atomic(int start, int end, const vector<int>& dane, atomic<int>& globalny_atomic) {
    for (int i = start; i < end; ++i) {
        if (dane[i] > 5000) {
            ...
            globalny_atomic.fetch_add(1); // To jest bardzo kosztowne!
        }
    }
}
...
int main() {
    ...
    watki.clear(); // Usuwamy wątki z pierwszego testu, aby użyć tego samego wektora
...
/* === ANALIZA WYDAJNOŚCI: LOKALNA AKUMULACJA VS GLOBALNY ATOMIC ===
 * * 1. METODA LOKALNA (Zalecana dla pętli):
 * - Każdy wątek używa własnej zmiennej 'local_count'.
 * - Dane są przechowywane w REJESTRACH procesora (najszybszy dostęp).
 * - Brak rywalizacji między rdzeniami - każdy pracuje niezależnie.
 * - Mutex/Atomic wywoływany TYLKO RAZ na samym końcu pracy wątku.
 * - WNIOSEK: Najwyższa wydajność i skalowalność.
 * * 2. METODA CIĄGŁEJ SYNCHRONIZACJI (Atomic wewnątrz pętli):
 * - Każda operacja 'fetch_add' wymusza komunikację z pamięcią RAM/Cache L3.
 * - Przy dużej liczbie trafień powstaje "korek" (Memory Contention).
 * - Rdzenie procesora muszą czekać, aż inne zakończą modyfikację zmiennej.
 * - WNIOSEK: Drastyczny spadek prędkości przy częstych aktualizacjach.
 * * ZŁOTA ZASADA: 
 * "Licz u siebie (lokalnie), melduj wynik na końcu (globalnie)".
 */